...
export default function VesselListClient() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0514] text-white font-mono p-8 pt-4 space-y-8">
      ...
      <div className="flex items-center gap-4">
        ...
        <button className="min-w-[300px] bg-[#150e24] border border-white/10 rounded-full py-3 px-5 flex items-center justify-between hover:border-[#bc66ff]/50 transition-all group">
          <div className="text-left">
            <p className="text-[8px] text-gray-500 uppercase tracking-[0.25em] font-bold">
              Fleet Control
            </p>
          ...
          <div className="w-8 h-8 rounded-full border border-white/10 flex items-center justify-center text-[#bc66ff] group-hover:rotate-90 transition-transform">
            +
          </div>
        </button>
      </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {[1, 2, 3].map((item) => (
            <div
              key={item}
              className="h-[420px] rounded-[2.5rem] bg-[#150e24] border border-white/5 animate-pulse"
            />
          ))}
        </div>
      ) : (
